SELECT str_to_date(concat(yearweek(`source`.`reviewed`), ' Sunday'), '%X%V %W') AS `date`, count(distinct `source`.`opportunity_id`) AS `opps_3mm_review_date_prime`
...
WHERE (`source`.`reviewed` > "2021-7-18"
   AND `source`.`reviewed` < date(now(6)) AND `Opportunities`.`fulfillment` = 'prime')
