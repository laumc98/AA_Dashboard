SELECT str_to_date(concat(yearweek(`opportunity_changes_history`.`created`), ' Sunday'), '%X%V %W') AS `created`, count(distinct `opportunity_changes_history`.`opportunity_id`) AS `count`
