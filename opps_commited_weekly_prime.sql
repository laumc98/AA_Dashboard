SELECT str_to_date(concat(yearweek(`opportunity_changes_history`.`created`), ' Sunday'), '%X%V %W') AS `date`, count(distinct `opportunity_changes_history`.`opportunity_id`) AS `opps_commited_weekly_prime`
...
WHERE (`opportunity_changes_history`.`type` = 'commit'
   AND `opportunities__via__opportunit`.`reviewed` >= date(date_add(now(6), INTERVAL -90 day)) AND `opportunities__via__opportunit`.`reviewed` < date(now(6)) AND `opportunities__via__opportunit`.`fulfillment` = 'prime')
