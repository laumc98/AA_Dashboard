SELECT str_to_date(concat(yearweek(`opportunities__via__opportunit`.`reviewed`), ' Sunday'), '%X%V %W') AS `opportunities__via__opportunit__reviewed`, count(distinct `opportunity_stats_hires`.`opportunity_id`) AS `count`
