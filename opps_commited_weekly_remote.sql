...
WHERE (`opportunity_changes_history`.`type` = 'commit'
   AND `opportunities__via__opportunit`.`reviewed` >= date(date_add(now(6), INTERVAL -90 day)) AND `opportunities__via__opportunit`.`reviewed` < date(now(6)) AND `opportunities__via__opportunit`.`remote` = TRUE)
