SELECT str_to_date(concat(yearweek(`source`.`match_date`), ' Sunday'), '%X%V %W') AS `date`, count(distinct `source`.`opportunity_id`) AS `opps_3mm_prime`
...
WHERE (`source`.`created` >= date(date_add(now(6), INTERVAL -90 day))
   AND `source`.`created` < date(date_add(now(6), INTERVAL 1 day)) AND `Opportunities`.`fulfillment` = 'prime')
